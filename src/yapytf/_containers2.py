...
def _get_generic_types(
    ...
    *,
    ...
    types = _typing_inspect.get_args(_typing_inspect.get_generic_type(self))

    if not types:
        types = _typing_inspect.get_args(_typing_inspect.get_generic_bases(self)[0])

    globalns = _sys.modules[self.__class__.__module__].__dict__
    ...
    if count != {-1} and len(types) not in count or any(_typing_inspect.is_typevar(i) for i in types):
        raise TypeError(f"{self.__class__.__name__} generic was not properly parameterized{hint}: {types}")
...
class _NestedDictGetter(_Getter[T]):
    ...
    def __init__(
        ...
        self._path: _tp.List[_PathComponent] = list(path)
        self._type = default
        concrete = _typing_inspect.get_origin(default) or default
...
class _ContextManagerMixin:
    ...
    def __enter__(self: Tself) -> Tself:
        ...
        self._context_copy = _tp.cast(Tself, _copy.copy(self))
...
def _make_const_ctor(type_: _tp.Type[_tp.Any]) -> _tp.Callable[[_tp.Any], _tp.Any]:
...
class _ConstRecordField(_tp.Generic[Tval]):
    __slots__ = ("path", "name", "val_ctor")

    def __init__(self, *, path: _tp.List[str] = [], name: _tp.Optional[str] = None) -> None:
        self.path = path
        self.name = name
    ...
    def __set_name__(self, owner: _tp.Type[Record], name: str) -> None:
        ...
        type_, = _get_generic_types(self, 1, hint=f". Field {name}")
        self.val_ctor = _make_const_ctor(type_)
    ...
    def __get__(self, instance: Record, owner: _tp.Type[Record]) -> Tval:
        ...
        container = _traverse_dict(
            instance._getter.get(),
            (_PathComponent(i, False) for i in self.path)
        )
        try:
            return _tp.cast(Tval, self.val_ctor(container[self.name]))
        except KeyError as e:
            raise AttributeError(e)
...
class ConstList(_tp.Sequence[T], _ContextManagerMixin):
    ...
    @_tp.overload  # noqa: F811
    def __getitem__(self, i: slice) -> _tp.Sequence[T]:
        ...

    def __getitem__(self, i: _tp.Union[int, slice]) -> _tp.Union[T, _tp.Sequence[T]]:  # noqa: F811
...
def _make_ctor(
    ...
    if isinstance(type_, tuple):
        ...
        def ctor(getter: _Getter[_DictAny], key: _tp.Any) -> _tp.Any:
            return class_(_NestedDictGetter(getter, [_PathComponent(key, is_attr)], class_._container_type()), types)
        ...
        def ctor(getter: _Getter[_DictAny], key: _tp.Any) -> _tp.Any:
            return rec_class(_NestedDictGetter(getter, [_PathComponent(key, is_attr)], _DictAny))
    ...
    if isinstance(type_, tuple):
        ...
        def setter(getter: _Getter[_DictAny], key: _tp.Any, val: _tp.Any) -> None:
            if isinstance(val, class_):
                ...
                obj = class_(
                    _NestedDictGetter(getter, [_PathComponent(key, is_attr)], class_._container_type()),
                    types
                )
                obj._populate(val)
...
class _RecordField(_tp.Generic[Tval, Tsetter]):
    ...
    def __set_name__(self, owner: _tp.Type[Record], name: str) -> None:
        ...
        types = _get_generic_types(self, {1, 2}, hint=f". Field {name}")
        if len(types) == 1:
            types = types * 2
...
class _List(_tp.MutableSequence[Tval], _tp.Generic[Tval, Tsetter], _ContextManagerMixin):
    ...
    def __getitem__(self, i: _tp.Union[int, slice]) -> _tp.Union[Tval, _tp.MutableSequence[Tval]]:  # noqa: F811
    ...
    @_tp.overload  # noqa: F811  # type: ignore
    def __setitem__(self, i: slice, v: _tp.Iterable[Tsetter]) -> None:
        ...

    def __setitem__(self, i: _tp.Union[int, slice], v: _tp.Union[T, _tp.Iterable[Tsetter]]) -> None:  # noqa: F811
        container = self._getter.get()
        if isinstance(i, slice):
    ...
    def __delitem__(self, i: _tp.Union[int, slice]) -> None:  # noqa: F811
